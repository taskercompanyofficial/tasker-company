...
export async function generateMetadata({
  ...
  const endPoint = `${API_URL}${CATEGORIES}/${slug}`;
  ...
  return {
    title: `${response.data.name} | Categories ${title}`, // Dynamic title
    description: response.data.description || { description },
    keywords: [keywords, keywordsArray],
    openGraph: {
      title: `${response.data.name} | Categories ${title}`, // Dynamic title
      description: response.data.description || { description },
      url: `https://taskercompany.com/categories/${slug}`, // Adjust URL to your app structure
